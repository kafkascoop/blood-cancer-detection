...
const predictionColor: Record<string, string> = {
  Normal: 'emerald',
  Benign: 'amber',
  Malignant: 'rose',
};
...
export default function History() {
  ...
  const itemTemplate = (r: DetectionResult) => (
    <div
      className="flex items-center justify-between p-4 bg-white rounded-xl border border-gray-200 cursor-pointer hover:border-blue-200 hover:shadow-sm transition-all mb-2"
      onClick={() => setSelectedResult(r)}
    >
      <div className="flex items-center gap-3">
        <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${
          r.prediction === 'Normal' ? 'bg-emerald-100 text-emerald-700' :
          r.prediction === 'Benign' ? 'bg-amber-100 text-amber-700' :
  ...
  if (selectedResult) {
    return (
      <div className="page">
        ...
        <Button label="Back to history" icon="pi pi-arrow-left" text className="mb-4" onClick={() => setSelectedResult(null)} />
